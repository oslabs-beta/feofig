const validateConfigs = (config) => {
    ...
    if (!config)
        throw new Error('No configuration file was provided!');
    ...
    if (invalidProps.length > 0) {
        throw new Error(`Invalid properties found in the Configuration file: ${invalidProps.join(', ')}`);
    }
    // validates "lazyload"
    if (config.lazyload) {
        ...
        if (threshold !== undefined &&
            ...
            throw new Error('Lazyload: threshold must be a valid number between 0 and 1');
        // validates "once"
        ...
        if (offset !== undefined &&
            ...
            throw new Error('Lazyload: offset must be a string containing a number followed by px. (Ex: "100px")');
    }
    ...
    if (config.debounce) {
        ...
        const invalidProps = actualProps.filter((prop) => !allowedProps.includes(prop));
        if (invalidProps.length > 0) {
            throw new Error(`Invalid properties found in the debounce configuration: ${invalidProps.join(', ')}`);
        }
        // validates "delay"
    ...
    if (config.throttle) {
        ...
        const invalidProps = actualProps.filter((prop) => !allowedProps.includes(prop));
        if (invalidProps.length > 0) {
            throw new Error(`Invalid properties found in the throttle configuration: ${invalidProps.join(', ')}`);
        }
        // validates "delay"
        if (delay !== undefined && (typeof delay !== 'number' || delay < 0))
            throw new Error('Debounce: delay must be a valid, positive number');
